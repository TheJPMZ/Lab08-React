...
import cardAudioFile from '../assets/Card-flip-sound-effect.mp3';
import dingAudioFile from '../assets/ding-sound-effect_2.mp3';
import useStateManager from './statemanager';

// Card art from: thegentlehoneybee

const images = [
  'https://64.media.tumblr.com/c6959d109e26493553c54c77712e5d7a/83593f888efdcaa0-1f/s540x810/e5d05285d5f00354a62e8756853889b4d26eefda.jpg',
...
function Card({ image }) {
  ...
  const cardAudio = new Audio(cardAudioFile);
  cardAudio.volume = 0.2;
  const dingAudio = new Audio(dingAudioFile);
  dingAudio.volume = 0.2;

  const rotate = () => {
    const flip = () => {
      cardAudio.play();
      cardRotate(!cardState);
      addMovement();
    };
  ...
  return (
    <div className={`card ${cardState ? 'rotated' : ''}`} onClick={rotate} onKeyPress={rotate} role="button" tabIndex={0}>
      <div className="card_position">
        <div className="carta_front" />
        <div className="carta_back" style={{ backgroundImage: string }}>
          {' '}
        </div>
      </div>
    </div>
